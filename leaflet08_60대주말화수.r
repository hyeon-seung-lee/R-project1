#2018년 60대이상, 주말 - (화,수) 차이가 가장 큰 곳을 나타내기
rm(list = ls())
...
flow18_weekend <-flow18[,c(6,269:280,521:532,534:536)]
...
#2018년 60대이상 화요일 수요일
load("save/flow18.rdata")
flow18_tuewedn <-flow18[,c(6,245:256,497:508,534:536)]
...
leaflet(flow18_weekend_total) %>% addTiles() %>%
  ...
  addCircles(lng = ~lon, lat = ~lat, weight = 1,
             ...
             color = ~flow_color(group), fillOpacity = 0.2, group = '60대이상 주말') %>% 
  ...
  addLayersControl(       # 옵션 컨틔롤
    ...
    overlayGroups = c("상권","서울 명소","지하철역", 
                      "60대이상 주말", "60대이상 화수", "60대이상 주말-화수", "60대이상 화수-주말"),
